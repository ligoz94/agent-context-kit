# Context Policy (agents)

How much documentation to load per turn. Goal: **correctness and traceability** without **context rot** (noise, distraction, contradiction).

Full reading order for humans remains in the app's `docs/README.md`. This file is the **tiering contract** for AI agents.

> **App-specific paths**: All file paths below use generic references. Resolve concrete paths via the app's `app-config.md` (see [§ App Config](#app-config)).
